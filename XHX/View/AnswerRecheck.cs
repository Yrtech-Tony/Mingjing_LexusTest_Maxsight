...
namespace XHX.View
{
    ...
    public partial class AnswerRecheck : BaseForm
    {
        ...
        {
            if (rdoNO.Checked && string.IsNullOrEmpty(txtRecheckContent.Text.Trim()))
            {
                CommonHandler.ShowMessage(MessageType.Information, "未通过必须填写复审意见");
                return;
            }
            string imageName = "";
            if (imageName == null)
            ...
            service.SaveRecheck(ProjectCode_Golbal, SubjectCode_Golbal, ShopCode_Golbal, this.UserInfoDto.UserID,
                                   txtRecheckContent.Text, reCheckTypeCode, passReCheck, score, false);

            //保存RecheckDtl
            List<ReCheckDtlDto> recheckDtlList = grcRecheckError.DataSource as List<ReCheckDtlDto>;
            if (recheckDtlList != null)
            {
                foreach (ReCheckDtlDto r in recheckDtlList)
                {
                    if (r.Selected == false)
                    {
                        service.DeleteRecheckDtl(ProjectCode_Golbal, SubjectCode_Golbal, ShopCode_Golbal, reCheckTypeCode, r.ErrorType);
                    }
                    else
                    {
                        service.SaveRecheckDtl(ProjectCode_Golbal, SubjectCode_Golbal, ShopCode_Golbal, reCheckTypeCode, this.UserInfoDto.UserID, r.ErrorType);
                    }
                }
            }
            //查询下一个问卷信息并显示
        ...
        {
            RecheckStatus();
            if (string.IsNullOrEmpty(txtOrder2.Text))
            {
                CommonHandler.ShowMessage(MessageType.Information, "请输入序号");
                return;
            }
        ...
        public void GetRecheckDtl(string projectCode, string shopCode, string subjectCode, string recheckType)
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    reCheckDtl.ErrorTypeName = Convert.ToString(ds.Tables[0].Rows[i]["ErrorTypeName"]);
                    reCheckDtl.Selected = Convert.ToBoolean(ds.Tables[0].Rows[i]["Selected"]);
                    reCheckDtlList.Add(reCheckDtl);
                }
            }
            grcRecheckError.DataSource = reCheckDtlList;
        }
