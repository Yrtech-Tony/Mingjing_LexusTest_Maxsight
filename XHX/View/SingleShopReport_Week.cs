    ...
    public partial class SingleShopReport_Week : BaseForm
    {
        ...
        private void WriteDataToExcel(ShopReportDto shopReportDto)
        {
            if (!checkBox1.Checked)
            {
                ...
                workbook.Close(true, Path.Combine(tbnFilePath.Text, "2017Q2" + "梅赛德斯-迈巴赫销售质量现场考核" + "_" + shopReportDto.ShopCode + "_" + shopReportDto.ShopName + "_单店报告" + ".xlsx"), Type.Missing);
            }
            else
            {
                Workbook workbook = msExcelUtil.OpenExcelByMSExcel(tbnFilePath.Text + @"\" + "雷克萨斯单店报告.xlsx");
                Worksheet worksheet_FengMian = workbook.Worksheets["封面"] as Worksheet;
                #region 经销商基本信息
                {

                    #region 经销商基本信息
                    msExcelUtil.SetCellValue(worksheet_FengMian, "E8", shopReportDto.ShopCode);
                    msExcelUtil.SetCellValue(worksheet_FengMian, "E10", shopReportDto.ShopName);
                    msExcelUtil.SetCellValue(worksheet_FengMian, "E12", shopReportDto.AreaName);
                    //msExcelUtil.SetCellValue(worksheet_FengMian, "J14", shopReportDto.SalesContant);


                    Worksheet worksheet_Subject = workbook.Worksheets["现场检查得分"] as Worksheet;
                    #endregion
                    #region 体系信息
                    for (int i = 2; i < 30; i++)
                    {
                        for (int j = 0; j < shopReportDto.ShopSubjectScoreInfoDtoList.Count; j++)
                        {
                            if (msExcelUtil.GetCellValue(worksheet_Subject, "C", i).ToString() == shopReportDto.ShopSubjectScoreInfoDtoList[j].SubjectCode
                                || msExcelUtil.GetCellValue(worksheet_Subject, "C", i).ToString() == "*" + shopReportDto.ShopSubjectScoreInfoDtoList[j].SubjectCode)
                            {
                                string content = shopReportDto.ShopSubjectScoreInfoDtoList[j].Score;
                                if (!string.IsNullOrEmpty(GetString(shopReportDto.ShopSubjectScoreInfoDtoList[j].LossDesc.Split(';'))))
                                {
                                    content = GetString(shopReportDto.ShopSubjectScoreInfoDtoList[j].LossDesc.Split(';'));
                                }
                                msExcelUtil.SetCellValue(worksheet_Subject, "H", i, content);
                                //msExcelUtil.SetCellValue(worksheet_Subject, "H", i, GetString(shopReportDto.ShopSubjectScoreInfoDtoList[j].LossDesc.Split(';')));
                ...
                if (shopReportDto.SaleContantScoreInfoList.Count > 0)
                {
                    ...
                    {
                        msExcelUtil.SetCellValue(worksheet_SaleContant, i + 8, 3, "销售顾问" + "\r\n" + shopReportDto.SaleContantScoreInfoList[i].SaleName);
                ...
                for (int i = 12; i < 100; i++)
                {
                    ...
                    {
                        if (msExcelUtil.GetCellValue(worksheet_SaleContant, "C", i).ToString() == shopReportDto.SaleContantSubjectScoreDtoList[j].SubjectCode
                            || msExcelUtil.GetCellValue(worksheet_SaleContant, "C", i).ToString() == "*" + shopReportDto.SaleContantSubjectScoreDtoList[j].SubjectCode)
                        {
                            //msExcelUtil.SetCellValue(worksheet_SaleContant, "O", i, shopReportDto.SaleContantSubjectScoreDtoList[j].Remark);
                            for (int z = 8; z < 15; z++)
                            {
                                if (msExcelUtil.GetCellValue(worksheet_SaleContant, z, 3).ToString()
                ...
                workbook.Close(true, Path.Combine(tbnFilePath.Text, CommonHandler.GetComboBoxSelectedValue(cboProjects).ToString() + " LEXUS销售流程明检" + "_" + shopReportDto.ShopCode + "_" + shopReportDto.ShopName + "_单店报告" + ".xlsx"), Type.Missing);
            }
        }
