    ...
    public partial class Shop : BaseForm
    {
        ...
        public override void ExcelDownButtonClick()
        {
            if (grcShop.DataSource != null)
                ...
                CommonHandler.ExcelExportByExporter(grvShop);
        }
                ...
                foreach (ShopDto shop in shopList)
                {
                    if (shop.StatusType == 'I' || shop.StatusType == 'U')
                    {
                        webService.SaveShop(shop.SaleSmall, shop.AfterSmall, shop.ShopCode, shop.ShopName.Trim(), shop.UseChk, shop.UserName, shop.Province, shop.City
                            , shop.SalesOrAftersales, shop.GroupName, shop.CarType);
                    }
                    else {
        ...
        private void SearchShop()
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    shop.ReportPassword = Convert.ToString(ds.Tables[0].Rows[i]["ReportPassword"]);
                    shopList.Add(shop);
                }
            }
        ...
        private void simpleButton2_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 2; i < 500; i++)
            {
                ...
                if (!string.IsNullOrEmpty(shopCode))
                {
                    ...
                    string areaCode = msExcelUtil.GetCellValue(worksheet_FengMian, "B", i);

                    try
                    {
                        webService.UpdateShopArea(shopCode,areaCode,"");
                    }
                    catch (Exception)
                    {
