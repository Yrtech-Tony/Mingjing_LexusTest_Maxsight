...
namespace XHX.View
{
    public partial class ReCheckProcess : BaseForm
    {
        ...
        {
            grcRecheckProcessDtl.DataSource = null;
        ...
        private void btnReturn_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            if (this.UserInfoDto.RoleType == "S")
            {
                if (CommonHandler.ShowMessage(MessageType.Confirm, "确定回到前一个状态吗？") == DialogResult.Yes)
                {
                    AnswerStatusTypeCodeDto answer = grvRecheckProcess.GetRow(grvRecheckProcess.FocusedRowHandle) as AnswerStatusTypeCodeDto;

                    service.DeleteRecheckStatus(CommonHandler.GetComboBoxSelectedValue(cboProjects).ToString(), answer.ShopCode, answer.StatusCode);
                }
                SearchAnswerLog();
            }
        }
